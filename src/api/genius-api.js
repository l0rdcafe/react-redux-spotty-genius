import { GENIUS_ENDPOINT, CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN } from "../constants/genius";
...
const getAnnotations = async ({ song, artist }) => {
  ...
  try {
    ...
    if (response.hits.length > 0) {
      const isSameArtist = artist.toLowerCase() === response.hits[0].result.primary_artist.name.toLowerCase();
      const isSameSong = song.toLowerCase() === response.hits[0].result.title.toLowerCase();
